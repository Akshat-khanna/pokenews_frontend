...
const Form = ({ currentId, setCurrentId }) => {
    const [playerData, setPlayerData] = useState({ pokemon_name: '', description: '', tags: '', selectedFile: '' })
    ...
    const clear = () => {
        setCurrentId(null)
        setPlayerData({ pokemon_name: '', description: '', tags: '', selectedFile: '' })
    }
        ...
        return (
            <Paper className={classes.paper}>
                <Typography variant="h6" align="center" style={{ color: '#B3B3B3' }}>
                    Please sign in to add your own poke cards.
                </Typography>
            </Paper>
        )
    }
    ...
    return (
        <Paper className={classes.paper} elevation={6}>
            ...
            <form autoComplete="off" noValidate className={`${classes.root} ${classes.form}`} onSubmit={handleSubmit}>
                <Typography variant="h6" style={{ color: 'white' }}>{currentId ? 'Editing' : 'Creating'} a Poke Card</Typography>
                <TextField
                    name="pokemon_name"
                    variant="outlined"
                    label="Pokemon Name"
                    fullWidth
                    value={playerData.pokemon_name}
                    onChange={(e) => setPlayerData({ ...playerData, pokemon_name: e.target.value })}
                    InputLabelProps={{
                        style: { color: '#B3B3B3' }
                    }} />
                <TextField
